...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy({x => x.toLower}).toList map {case (x,y) => (x, y.length)} sortWith(_._1 < _._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val flat = s.flatten.mkString
    wordOccurrences(flat)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def helper(curr: Occurrences, acc: Occurrences): List[Occurrences] = curr match {
      case List() => List(acc sortWith(_._1 < _._1))
      case (x, 0)::xs => helper(xs, acc)
      case (x, y)::xs => helper(xs, (x, y)::acc) ++ helper((x, y-1)::xs, acc)
    }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def helper(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(Nil)
        ...
        sentence <- helper(subtract(occ, wordOccurrences(word)))
        if comb.nonEmpty
      } yield word :: sentence
